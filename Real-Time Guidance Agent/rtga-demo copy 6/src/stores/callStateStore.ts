...
interface CallState {
  ...
  // New: Tool executions and search results for Agent Hub
  ...
  // New: Tool and search actions
  executeTool: (toolType: AgentToolType, toolName: string, input?: string) => void
  addSearchResult: (query: string, answer: string) => void
...
const toolResponses: Record<AgentToolType, (input?: string) => string> = {
  crm: (input) => `Customer record updated in CRM. ${input ? `Added note: "${input}"` : 'Contact flagged for follow-up.'}`,
  search: (input) => `Found 3 relevant results for "${input || 'query'}". Top result: Knowledge base article on card replacement procedures.`,
  email: (input) => `Email draft created and queued for "${input || 'customer'}". Will be sent after call completion.`,
  calendar: (input) => `Follow-up reminder scheduled${input ? ` for ${input}` : ' for tomorrow at 10:00 AM'}.`,
  notes: (input) => `Note saved to call record${input ? `: "${input}"` : '.'}`,
}
...
export const useCallStateStore = create<CallState>((set) => ({
  ...
  executeTool: (toolType, toolName, input) => set((state) => {
    const execution: ToolExecution = {
      ...
      toolType,
      input,
      result: toolResponses[toolType](input),
      status: 'success',
  ...
  addSearchResult: (query, answer) => set((state) => {
