...
import { User, Headphones, MousePointerClick } from 'lucide-react'
...
export function ConversationPanel() {
  ...
  const { nextMessage, canAdvance } = useSimulation()
  ...
  return (
    <div
      className="flex-1 overflow-y-auto p-4 space-y-4 cursor-pointer relative"
      onClick={handleClick}
      title={canAdvance ? "Click to advance conversation" : ""}
    >
      {/* Click hint overlay */}
      {canAdvance && displayedMessages.length === 0 && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          <div className="flex flex-col items-center gap-2 text-muted-foreground">
            ...
            <span className="text-sm font-medium">Click anywhere to start</span>
          </div>
        </motion.div>
      )}
